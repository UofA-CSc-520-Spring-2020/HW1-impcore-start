...
(class CoordPair
   [subclass-of Object]
   [ivars x y] ; two numbers         ;;;;;; Representation, as instance variables
   ...
   (method + (coordPair) (CoordPair withX:y: (x + (coordPair x)) (y + (coordPair y))))
   (method - (coordPair) (CoordPair withX:y: (x - (coordPair x)) (y - (coordPair y))))
)
;;;usm.nw:859
(class Picture
    ...
    (method renderUsing: (aCanvas)
        ...
        (shapes do: [block (shape) (shape drawOn: aCanvas)])  ; <- draw each shape
        (aCanvas stopDrawing)
        self)
)
;;;usm.nw:1010
(class TikzCanvas 
    ...
    [subclass-of Object]
    (method startDrawing () 
        ('\begin print) (left-curly print) ('tikzpicture print) (right-curly print)
        (left-square print) ('x=4pt,y=4pt print) (right-square println))

    (method stopDrawing ()
        ('\end print) (left-curly print) ('tikzpicture print) (right-curly println))
    ...
    (method drawEllipseAt:width:height: (center w h)
        ...
        (left-round print)
            ((w div: 2) print) (space print) ('and print) (space print) ((h div: 2) print)
        (right-round print)
        (semicolon println))
)
;;;usm.nw:1238
