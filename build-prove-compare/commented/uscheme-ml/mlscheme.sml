...
(*  primitives that share code. There's no need to *)
(*  resort to tags.                             *)
...
fun eval (e, rho) =
  ...
  let fun ev (LITERAL v) = v
        ...
        (* pointer to a location allocated on the heap. In C, *)
        ...
        (* primitive function, as in C, I apply it to the *)
 ...
 (* <if any expression in [[bs]] is not a [[lambda]], reject the [[letrec]]>= *)
...
(* produces a value. \stdbreak To make the evaluator *)
(* easy to write, I do most of the work of evaluation in *)
(* the nested function [[ev]], which inherits the \ *)
(* stdbreak environment [[rho]] from the outer function *)
(* [[eval]]. \stdbreak Most AST nodes are evaluated in *)
(* the same environment as their parents, and each such *)
(* node is evaluated by passing it to [[ev]], which lets *)
(* [[rho]] be implicit. The first case of [[ev]] is the *)
(* evaluation of a literal value [[v]], which evaluates *)
(* to itself. \mlsflabeleval                    *)
(* <boxed values 4>=                            *)
...
(* stdbreak Given a function [[f]] of type \monoboxvalue *)
(* list -> value, \stdbreak [[inExp f]] is a primitive *)
(* function that also takes an [[exp]] as argument. The *)
(* new function applies [[f]], and if applying [[f]] *)
(* raises the [[RuntimeError]] exception, it adds the *)
(* expression to the error message.             *)
(* <boxed values 9>=                            *)
...
(* micro-Scheme has two kinds of predicate: an ordinary *)
(* predicate takes one argument and a comparison takes *)
(* two. Some comparisons apply only to integers. An ML *)
(* predicate returns a [[bool]], but a micro-Scheme *)
(* predicate needs to return a [[value]], so we compose *)
(* each predicate with [[embedBool]].           *)
(* <boxed values 12>=                           *)
  ...
  let val rho =
        foldl (fn ((name, prim), rho) => bind (name, ref (PRIMITIVE (inExp prim)
              ...
              emptyEnv ((* I organize my primitives into a list of (name, *)
                     ...
                     (* stdbreak Each primitive on the list has type \monobox *)
                        (* value list -> value. \stdbreak In \chunkref  *)
                        (* mlscheme.inExp-applied, I apply [[inExp]] to every *)

                       (* primitive on the list, then use the result to build *)
                        (* micro-Scheme's initial environment. [Actually, the *)
                        (* list contains all the primitives except one. The *)

                       (* exception is [[error]], which should not be wrapped *)
                        (* in [[inExp]].] Here are the first four elements: *)
