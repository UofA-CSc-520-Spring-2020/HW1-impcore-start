...
Value eval(Exp e, Env env) {
     ...
     * to [[env]]. There's just a little sanity
     * checking---if someone changes \uschemeplus variable
     * [[ --- trace-stack]] from a number to a non-number,
     * chaos may ensue.
     * <use the options in [[env]] to initialize the instrumentation>=
     */
    ...
    exp: 
        ...
        case WHILEX:
            /*
             * Interpreting the \xwhile loop
             * 
             * Now we can forget about lists of expressions and move
             * on to loops and control operators. We start
             * evaluating a \xwhile loop by pushing a \xwhile
             * context onto the stack and evaluating the condition.
            ...
            if (/*
                ...
                switch (e->u.letx.let) {
                   ...
                   case LETREC:
                       ...
                       */
                      /*
                       * <if not all of [[e->u.letx.es]] are lambdas, reject the
                                                                    [[letrec]]>=
                       */
    ...
    value: 
        ...
        if (fr == NULL) {
            ...
            case WHILE_RUNNING_BODY:
                /*
                 * EQ is an operation that compares symbols or numbers
                 * for equality and returns TRUE or FALSE\footnotemark.
                 * Thus, [As compared with =, which returns T or (). EQ
                 * can be defined in Haskell as: {centercode} (set EQ
                 * (lambda (x y) (if (= x y) TRUE FALSE))) {centercode}
                 * We will later see how to define equality on integers
                 * separately, using EQ only for symbols.]
            ...
            case APPLY:
                ...
                apply_last_arg:   // Small-Step-Apply-Last-Arg (or no arguments)
                    /*
                     * Once the overwritten [[fr->context.u.apply.actuals]]
                     * and [[fr->context.u.apply.fn]] are converted to
                     * values, we free their memory. The third case
                     * implements the call in the standard way.
