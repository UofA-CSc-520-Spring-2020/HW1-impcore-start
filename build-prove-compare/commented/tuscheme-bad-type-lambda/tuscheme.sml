...
(*                                              *)
(* The code in this section is a late addition to the *)
(* Supplement. Some of it ought to migrate into \cref *)
(* mlinterps.chap.                              *)
(*                                              *)
...
(* To build types in Typed uScheme, we use type-level *)
(* expressions. These include not only type variables *)
(* and the \astforall quantifier but also type  *)
(* constructors, constructor application, and   *)
(* function types. In my ML code, I represent the *)
(* abstract syntax of type-level expressions using the *)
(* type [[tyex]]. \tuslabeltyex                 *)
(* <types for {\tuscheme}>=                     *)
...
(* ``types''---unless we're being careful.      *)
...
(*                                              *)
(* This code prints types. It might be desirable to *)
(* print them using a more ML-like syntax.      *)
(* <types for {\tuscheme}>=                     *)
...
(* Predefined recursive, polymorphic functions  *)
(* [[filter]] and [[map]] [*]                   *)
(* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ *)
(*                                              *)
...
(* shows how a type system based on quantified types is *)
(* combined with micro-Scheme to produce Typed uScheme. *)
(*                                              *)
...
(*                                              *)
(* As in Chapter [->], we define modulus in terms of *)
(* division.                                    *)
...
(* <substitute in [[tau]] as specified by [[varenv]], without capturing or substituting for any [[alphas]]>= *)
            ...
            in  if null actual_captures then
                  ...
                  (* not substitute for a bound variable and that we avoid *)
                  (* capturing any variables. Postponing for the moment *)
                  (* the issue of capture, we can prevent substitution for *)
                  (* a bound variable by extending [[varenv]] so that each *)
                  (* bound variable is mapped to itself:          *)
...
(* Infinite stream of type variables            *)
(*                                              *)
(* Stream [[infiniteTyvars]] is built from stream *)
(* [[naturals]], which contains the natural numbers; *)
(* [[naturals]] is defined in \chunkref         *)
(* mlinterps.chunk.naturals in \crefmlinterps.chap. *)
(* <boxed values 158>=                          *)
val _ = op naturals       : int stream
...
fun eqType (TYVAR a, TYVAR a') = a = a'
  ...
  | eqType (FORALL (alphas, tau), FORALL (alphas', tau')) =
      ...
      (* . \stdbreak I compare them by renaming the bound *)
      (* variables on both sides to \ldotsnbeta. \stdbreak *)
      (* Internal function [[ok]] ensures that no beta_i is *)
      (* free in either tau or tau'. [Because I~rename \emph *)
      (* {all} the $\alpha_i$'s and $\alpha_i'$'s, I~don't *)
      (* have to worry about a $\beta$ colliding with an *)
      (* existing $\alpha_i$~or~$\alpha'_i$.] After renaming, *)
      (* I compare \nomathbreak\/\ldotsnbeta\alldottau[\ *)
      (* ldotsmapstonalphabeta] with \nomathbreak\/\ldotsnbeta *)
      (* \alldottau'[\mapdotsnalpha'beta]. \stdbreak Rule \ *)
...
(* there is only one rule that applies and one way to *)
(* check equivalence. \tusflabeleqType,eqTypes [*] *)
(* <boxed values 2>=                            *)
...
(*                                              *)
(*   \toprule Type   Concept              Interpreter *)
(*       system                                 *)
(*     \midrule d    Definition           def (\cpagereftuscheme.type.def) *)
...
fun eval (e, rho) =
  ...
  let fun ev (LITERAL n) = n
        ...
        (* The rest of the evaluator appears in \cref   *)
        (* app:tuscheme.                                *)
...
(* [[e]] must not evaluate [[name]]---because the *)
(* mutable cell for [[name]] does not yet contain its *)
(* correct value.[*]                            *)

(* <definitions of [[basis]] and [[processDef]] for {\tuscheme}>= *)
(* \tuschemeheader's basis                      *)
(*                                              *)
(* A basis comprises a kind environment, a type *)
(* environment, and a value environment.        *)
...
(* Primitive values are made using functions    *)
(* [[unaryOp]], [[binaryOp]], and [[arithOp]], just as *)
(* in \crefmlscheme.chap. The only difference is that if *)
(* something goes wrong, the Typed uScheme versions *)
(* don't raise the [[RuntimeError]] exception;  *)
(* they raise [[BugInTypeChecking]]. And the primitives *)
(* also need types. For the type of the arithmetic *)
(* primitives, I define [[arithtype]].          *)
(* <boxed values 11>=                           *)
val _ = op unaryOp  : (value         -> value) -> (value list -> value)
val _ = op binaryOp : (value * value -> value) -> (value list -> value)
val _ = op arithOp  : (int * int -> int)       -> (value list -> value)
...
val primBasis =
  let fun addKind ((name, kind), kinds) =
      ...
      val (types, values) = foldl addPrim (emptyEnv, emptyEnv)
                            (
                         (* As in Chapter [->], I use the chunk [[<<primitive *)

                     (* functions for Typed uScheme [[::]]>>]] to cons up all *)

                        (* the primitives into one giant list, and I use that *)

                     (* list to build the initial basis. Each primitive has a *)
...
val initialBasis =
 ...
 "(val list1 (type-lambda ['a] (lambda ([x : 'a]) ([@ cons 'a] x [@ '() 'a]))))"
          ...
          "                               ([@ cons 'a] x ([@ list2 'a] y z)))))"
                       ...
                       , ""
                       , ";  These predefined functions are also written using "
                       , ";  [[type-lambda]]:                             "
                       , ";  <predefined {\\tuscheme} functions>=          "
                      ...
                      "      (lambda ([x : 'a]) (lambda ([y : 'b]) (f x y))))))"
                       ...
                       , ";  \\stdbreak The functions above are not recursive. "
                       , ";  To define a function that is both recursive and "
                       ,
                       ";  polymorphic, we nest [[letrec]] (for the recursion) "
                       , ";  inside [[type-lambda]] (for the polymorphism). "
                       ,
                      ";  The whole appears on the right-hand side of [[val]], "
                       , ";  as in this definition of [[length]]:[*]      "
                       , ";  <predefined {\\tuscheme} functions>=          "
                       , "(val length"
                       ,
  "  (type-lambda ['a]                 ; value of thing: a polymorphic function"
                     ...
                     ";  using the same pattern: [[val]] to [[type-lambda]] to "
                       ,
                     ";  [[letrec]]. As another example, here is an explicitly "
                       , ";  typed version of the reverse-append function: "
                       , ";  <predefined {\\tuscheme} functions>=          "
                       , "(val revapp"
                       , "  (type-lambda ['a]"
                       ,
...
"                     ((@ cons 'a) ((@ car 'a) xs) (append-mono ((@ cdr 'a) xs) ys)))))]"
                       , "        append-mono)))"
                       ,
                       ";  Integer comparisons are as in Typed Impcore, but to "
                       , ";  define [[!=]] we need a type abstraction. This is "
                       , ";  progress! In Typed Impcore, a polymorphic [[!=]] "
                       , ";   can't be defined as as function.            "
                       , ";  <predefined {\\tuscheme} functions>=          "
