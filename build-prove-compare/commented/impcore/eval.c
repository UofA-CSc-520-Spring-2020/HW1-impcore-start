...
Value eval(Exp e, Valenv globals, Funenv functions, Valenv formals) {
    ...
    case VAR:
         ...
         * isvalbound(e->u.var, formals). If x \notindom rho and
         * x \notindom xi, the operational semantics gets
         * stuck---so the interpreter issues an error message.
         * Less formally, we look up the variable by checking
         * first the local environment and then the global
         * environment.
    ...
    case IFX:
         ...
         * recursive call \monoboxeval(e->u.ifx.cond, globals,
    ...
    case APPLY:
         ...
         */
        {
            ...
            case USERDEF:
                 ...
                 * I use \monoboxmkValenv(xs, vs) to create a fresh
            ...
            case PRIMITIVE:
                 ...
                 */
                {
                    ...
                    if (f.u.primitive == strtoname("print"))
                        ...
                        /*
                         * Because the name of each arithmetic primitive has
                         * just one character, I switch on that character. This
                         * technique is reasonable only because the set of
