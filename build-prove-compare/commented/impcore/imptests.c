...
TestResult test_result(UnitTest t, Valenv globals, Funenv functions) {
    ...
    case CHECK_ERROR:
        /*
         * To run a [[check-error]], we use the same tools in
         * different ways. Again we evaluate an expression under
         * the protection of an error handler, but now, if an
         * error occurs, the test passes. If not, it fails.
