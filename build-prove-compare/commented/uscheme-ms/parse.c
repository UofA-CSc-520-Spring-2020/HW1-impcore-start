...
/*
 * A [[let*]] expression can be syntactic sugar for a
 * sequence of nested [[let]] expressions. The syntactic
 * sugar for [[let*]] is defined by structural induction
 * on the list of bindings in the [[let*]], so to define
 * it, I write two equations: one for the base case with
 * no bindings, and one for the induction step, with a
 * nonempty sequence of bindings. \stdbreak An empty
 * [[let*]] is replaced by its body. A nonempty [[let*]]
 * is replaced by a nested sequence of [[let]]
 * expressions; in particular, it is a [[let]]
 * expression for the first binding, followed by the
 * desugaring of the [[let*]] expression for the
 ...
 * itself defined \stdbreak in terms of [[lambda]], so
 * that everything reduces to a lot of [[lambda]]s. \
 * stdbreak But what's happening here is no different
 * from what's happening in any other recursive
 * function---it's just that here, the recursive
 * function is applied to syntax, not to values. Here's
 * what the recursive function looks like: [*]
 * <parse.c>=
 */
