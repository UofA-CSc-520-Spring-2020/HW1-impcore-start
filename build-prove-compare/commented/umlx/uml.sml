...
(* Most of the type components are shared with either \ *)
(* nml or micro-Haskell.                        *)
(* <Hindley-Milner types with generated type constructors>= *)
...
(* whenever a generative type definition is elaborated. *)
...
(*                                              *)
(* To implement these rules in a way that doesn't make *)
(* my head hurt, I define an algebraic data type that *)
(* shows the four possible shapes of sigma, so I can *)
(* pattern match on them. The shapes are \mathbox\ *)
(* crossdotsntau-->tau, tau, \mathbox\/\ldotskalpha. \ *)
(* crossdotsntau-->tau, and \mathbox\/\ldotskalpha. tau. *)
(* <boxed values 97>=                           *)
...
(* Function [[extendTypeEnv]] takes a [[type_env]] on *)
(* the left but a \monoboxtype_scheme env on the right. *)
(* <boxed values 103>=                          *)
...
(* constructor. I want only to add a skolem type to an *)
(* existing set, not to allocate multiple sets, so I *)
(* begin with a function that can be passed to  *)
(* [[foldl]].                                   *)
(* <boxed values 110>=                          *)
...
(* MISPLACED: We begin our tour of syntax with type *)
(* expressions: a type expression in uML is just like a *)
(* type expression in Typed uScheme \tustypepagetyex. *)
(* But in Typed uScheme, the name of a type (or type *)
...
(* My substitution into Gamma' is just good enough for *)
(* patterns---I know that every type scheme in Gamma' is *)
(* a monotype.                                  *)
(* <boxed values 111>=                          *)
...
fun exptable exp =
  ...
  in  (* Before the first call to [[allocate]], a client must *)
...
(* is looked up in the environment Delta. [*] \umlflabel *)
(* txType                                       *)
(* <boxed values 99>=                           *)
...
(* <definition of [[validate]], for the types of the value constructors of [[T]]>= *)
      fun validate (K, sigma as FORALL (alphas, _), mu, kind) =
        let (* <definitions of [[appliesMu]] and [[validateTypeArguments]]>= *)
      ...
      val _ = op validateLengths       : tyvar list * kind list -> unit
        in  (* When we have value constructors with existential *)
            (* types, additional validation is needed.      *)

(* <validation by case analysis on [[schemeShape shape]] and [[kind]] ((existentials))>= *)
...
(* These two cases aren't the only good cases; when an *)
(* algebraic data type takes type parameters, its value *)
(* constructors are polymorphic, and there are two more *)
(* cases, making four in total: {mathpar} \vconcompat *)
(* sigmaµ\akind                                *)
(*                                              *)
(* \tyrule MonoValCompat \vconcompatµµ\ktype  *)
(*                                              *)
(* \tyruleMonoFunCompat \vconcompat\crossdotsntau-->µ µ\ *)
(* ktype                                        *)
(*                                              *)
(* \tyrule PolyValCompat \ldotskprimealpha all distinct *)
(* \vconcompat\/\ldotskalpha. (\ldotskprimealpha) µ µ\ *)
(* cdotsk\ktype\karrow\ktype                    *)
(*                                              *)
(* \tyrule PolyFunCompat \ldotskprimealpha all distinct *)
(* \vconcompat\/\ldotskalpha. \crossdotsntau-->(\ *)
...
(* the compatibility judgment, it's enough to check that *)
(* µ is applied to distinct type variables. The *)
(* translation judgment \txtyscheme[Delta'] t_i sigma_i *)
(* ensures that only type variables which appear under *)
(* the \/ can be used.                          *)
(*                                              *)
...
(* translation judgment describe below, here is the *)
(* function that elaborates a [[data]] definition. The *)
(* function returns Gamma', Delta', and a list of *)
(* strings: the name \stycon followed by names [\ldotsn *)
(* K]. \umlflabelelabDataDef                    *)
(* <boxed values 85>=                           *)
...
(* [*] \typesystemuml                           *)
(*                                              *)
(* Just as in Typed uScheme, we never trust type syntax *)
(* written by a programmer: we have to rule out nonsense *)
(* like \monobox(int int) or \monobox(list -> list). *)
(* In Typed uScheme, it suffices to check the kind of *)
(* each type, then pass the syntax on to the type *)
(* checker. But the syntax of types used in Typed *)
(* uScheme is structured differently from the internal *)
(* Hindley-Milner types used for type inference. I deal *)
(* with this difference in structure by translating one *)
(* to the other.                                *)
(*                                              *)
(* The translation is expressed using judgment forms \ *)
(* txTypet tau\akind and \txTySchemet sigma, which *)
(* respectively translate uML type syntax into a *)
(* Hindley-Milner type or type scheme. The rules are *)
(* shown in \crefadt.fig.kinding. The implementation is *)
(* similar to Typed uScheme's kind checking, and at this *)
(* point you have plenty of practice reading inference *)
(* rules, so I doubt you need to see it. But if you do, *)
(* you will find it in \crefadta.chap.          *)
(*                                              *)
(* Run-time representation of value constructors *)
(*                                              *)
(* Data-type definitions are not only typechecked; they *)
(* are also evaluated. Evaluating a datatype definition *)
(* introduces a function or value for every value *)
(* constructor. A value constructor with a function type *)
(* gets a function; \stdbreak a value constructor with a *)
  ...
  let val predefinedTypes = 
                     ...
                     ";  [[println]] and [[print]], is inhabited by the single "
                          ...
                          ";  The [[order]] type is a standard result type for "
                             ,
                        ";  comparison functions. If a set is totally ordered, "
                             ,
                       ";  then any one element is either less than, equal to, "
                             ,
                              ";  or greater than another.                     "
                             , ";  <predefined uML types>=                   "
                             , "(implicit-data order LESS EQUAL GREATER)"
                             ,
                     ";  Update function [[project]] so it uses the coordinate "
                             ,
                              ";  to index into the point's list of integers.  "
                             ,
                       ";  Update your nearest-point function to work with the "
                             ,
                              ";  new representations.                         "
                             ,
                       ";  If you've completed \\crefadta.ex.k-nearest, update "
                             ,
                        ";  your nearest-k-point function to work with the new "
                             ,
                              ";  representations.                             "
                             ,
                     ";  Define a function that given a number N and a list of "
                             ,
                      ";  N-dimensional points, builds a suitable search tree. "
                             ,
                              ";  A good place to start is with a list of      "
                             ,
                     ";  [[coord-funs]] of length N. If you complete this data "
                             ,
                              ";  structure, you can use it as part of a       "
                             ,
                     ";  k-nearest-neighbor classifier. Such a classifier is a "
                             ,
                              ";  simple machine-learning tool, but still very "
                             ,
                     ";  effective on some problems, like classifying gestures "
                             ,
                      ";  based on a photograph of the human body. And as long "
                             ,
                         ";  as the number of dimensions is not too great, the "
                             ,
                       ";  search tree is reasonably efficient---it works well "
                             ,
                      ";  provided the number of points being searched is much "
                             ,
                              ";  larger than 2^N.                             "
                             ,
                              ";                                               "
             ...
             "  [TRIPLE : (forall ('a 'b 'c) ('a 'b 'c -> (triple 'a 'b 'c)))])"
                          ...
                          ";  When defining larger tuples, the notation of the "
                             ,
                      ";  explicit form is a bit much. I shift to the implicit "
                             ,
                              ";  form. [*]                                    "
                             , ""
                             , ";  <predefined uML types>=                   "
...
(* <function [[literal]], to infer the type of a literal constant ((prototype))>= *)
  ...
  (* <function [[literal]], to infer the type of a literal constant ((adt))>= *)
      (* <definition of function [[pvconType]] ((existentials))>= *)
      ...
      (* it needs a special version of [[extend]], called *)
      (* [[extendTypeEnv]], which is defined in the   *)
      (* Supplement.                                  *)
      (*                                              *)
...
(* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ *)
      ...
      fun choicetype ((p, e), Gamma) =
            let val (Gamma', tau, c) = pattype (p, Gamma)
                ...
                val _ =
                     (* \usetyExistentialChoice The rule is implemented using *)
                        (* these representations: {indented}            *)
                        (*                                              *)

                      (*   p     e     Gamma     Gamma'    tau-->  \tyc\land\ *)
                        (*                                 tau'      tyc' *)

                       (* [[p]] [[e]] [[Gamma]] [[Gamma']]  [[ty]]    [[con]] *)
                        (*                                              *)
                        (* {indented} To find the free skolem types of \subsn *)

                     (* Gamma', I look at all the types bound in \subsnGamma' *)

                       (* . But to find the free skolem types of \subsnGamma, *)
                        (* I need to look only at what \subsn substitutes for *)
                        (* the free type variables of Gamma.            *)

(* <check [[p]], [[e]], [[Gamma']], [[Gamma]], [[ty]], and [[con]] for escaping skolem types ((existentials))>= *)
                        let val theta = solve con
                                (* if exn is raised here, we're doomed anyway *)
                            val patSkolems = typeSchemesFreeSkolems (map snd (
                                                     typeEnvSubst theta Gamma'))
                            val envSkolems = typesFreeSkolems (map (varsubst
                                                 theta) (freetyvarsGamma Gamma))
                            val tySkolems  = typeFreeSkolems  (tysubst theta ty)
                            (* All the failure modes identify the problematic *)
                            (* pattern match and raise [[TypeError]].       *)

               (* <definitions of skolem functions [[fail]] and [[badType]]>= *)
                            fun fail ss =
                            ...
                            fun badType tau =
                              concat ["type ", typeString tau, ", which ",
                                      case tau of TYCON _ => "is" | _ =>
                                         "includes", " an escaping skolem type"]
                        in  case (inter (patSkolems, tySkolems), inter (
                                                        patSkolems, envSkolems))
                              of (mu :: _,  _) =>
                        (* If tau-->tau' has an escaping skolem type, I check *)

                              (* tau' first, then tau.                        *)

                              (* <fail with skolem escaping into type>=       *)
                               ...
                               "scrutinee is constrained to have ", badType tau]
                                                      | _ => let exception
                                 ChoiceTypeNotFun in raise ChoiceTypeNotFun end)
                               | ([], mu :: _) =>
                     (* If the problem is in the environment, I don't provide *)

                              (* much help.                                   *)

                             (* <fail with skolem escaping into environment>= *)
                                                  fail ["skolem type " ^
                   tyconString mu ^ " constrains a variable in the environment"]
                               | ([], []) => ()
                        end          
...
(* <function [[ty]], to infer the type of a \nml\ expression, given [[Gamma]]>= *)
      fun ty (LITERAL n) = literal n
        ...
        (* algorithm, we introduce constraints. Just as in \cref *)
        (* ml.chap, each instance of an unknown type is *)
        (* represented by a fresh type variable, and within each *)
        (* rule, multiple instances are constrained to be equal. *)
        (* The rules are shown in \vref                 *)
        (* adt.fig.exp-constraint-rules. To implement the *)
        (* algorithm, uML uses the same [[ty]] representation *)
        (* as \nml.                                     *)
        (*                                              *)
        (* \typesystemuml-constraints                   *)
        (*                                              *)
        (* To implement the \rulenameCase rule, I use [[typeof]] *)
        (* to check the scrutinee, and [[choicetype]] to check *)
        (* each choice (see \crefpageadt.rosetta-stone). \usety *)
        (* Case To ensure that each choice has the same arrow *)
        (* type, I define and use internal function     *)
        (* [[constrainArrow]]. \umlflabelty [*]         *)
        (* <boxed values 89>=                           *)
...
val initialBasis =
  let val predefinedFuns =
         ...
         , ";                                               "
         , ";  Algebraic data types are most often used in  "
         , ";  situations where there is a choice---where there is "
         , ";  more than one value constructor. But it is also "
         , ";  useful to define an algebraic datatype that has just "
         , ";  a single constructor. For example, the functions "
         , ";  [[pair]], [[fst]], and [[snd]], which in \\nml have to "
         , ";  be implemented as primitives (\\crefpageml.ex.pair), "
         , ";  in uML can be defined using user code: \\umllabelpair "
         , ";  <predefined uML functions>=               "
         , "(data (* * => *) pair"
         ...
         , "(define snd (p)"
         , "   (case p [(PAIR _ y) y]))"
         , ";  (We rarely need functions [[fst]] or [[snd]], except "
         , ";  perhaps to pass to [[map]], but they are included for "
         , ";  compatibility with \\nml.)                    "
         , ""
         , ";  Here's how these values are used when comparing "
         , ";  integers: \\addboxumlcompare-ints : (int int -> order) "
         , ";  <predefined uML functions>=               "
         ...
         , ";  so therefore the basic list functions must be built "
         , ";  in as primitives. In uML, the basic list functions "
         , ";  are written in user code. \\addboxumlnull? : (forall "
         , ";  ['a] ((list 'a) -> bool)) \\addboxumlcar : (forall "
         , ";  ['a] ((list 'a) -> 'a)) \\addboxumlcdr : (forall ['a] "
         , ";  ((list 'a) -> (list 'a)))                    "
         , ";  <predefined uML functions>=               "
         , "(define null? (xs)"
         , "   (case xs ['()        #t]"
         ...
         , "            [(cons _ ys) ys]))"
         , ";;unboxuml"
         , ";  But don't get too fond of [[null?]], [[car]], and "
         , ";  [[cdr]]. Part of the joy of algebraic data types is "
         , ";  that we rarely need these functions---instead we use "
         , ";  pattern matching. For example, here are versions of "
         , ";  [[append]] and [[revapp]] that don't use [[null?]], "
         , ";  [[car]], or [[cdr]]. \\addboxumlappend : (forall ['a] "
         , ";  ((list 'a) (list 'a) -> (list 'a))) \\addboxumlrevapp "
         ...
         , "            (cons p (bind x y ps)))]))"
         , ";;unboxuml"
         , ";  We can also do better with [[find]] and [[bound?]]. "
         , ";  In [[find]], if a key is not found, we needn't call "
         , ";  [[error]]; we can instead return a value of  "
         , ";  [[option]] type. Using nested patterns, we can "
         , ";  implement [[find]] without using [[fst]] or [[snd]]. "
         , ";  \\addboxumlfind : (forall ['a 'b] ('a (list (pair 'a "
         , ";  'b)) -> (option 'b)))                        "
         , ";  <predefined uML functions>=               "
         , "(define find (x alist)"
         , "  (case alist"
         ...
         , "              (find x pairs))]))"
         , ";;unboxuml"
         , ";  In \\nml, [[find]] and [[bound?]] require us to "
         , ";  implement the same search algorithm in two different "
         , ";  functions. But in uML, we implement the algorithm "
         , ";  once, in [[find]], and [[bound?]] calls [[find]]. \\ "
         , ";  addboxumlbound? : (forall ['a 'b] ('a (list (pair 'a "
         , ";  'b)) -> bool))                               "
         , ";  <predefined uML functions>=               "
         ...
         , ";                                               "
         , ";  [*] Some functions are exactly as in micro-Scheme or "
         , ";  \\nml.                                        "
         , ";  <predefined uML functions>=               "
         ...
         , ";  List functions are simpler with pattern matching. "
         , ";  Compare this code with \\crefpagemla.basis.   "
         , ";  <predefined uML functions>=               "
         , "(define filter (p? xs)"
         , "  (case xs"
         , "     ('()   '())"
