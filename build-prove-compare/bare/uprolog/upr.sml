(* upr.sml (BUG: software can't tell where this code came from [NWFRe5t-4FUkzb-1]) *)
...
val reverse = rev
(* list functions not provided by \sml's initial basis S80d *)
fun optionList [] = SOME []
...
(* simple implementations of set operations S79a *)
type 'a set = 'a list
val emptyset = []
fun member x = 
...
(* collections with mapping and combining functions S79b *)
...
(* streams (BUG: software can't tell where this code came from [NWFRe5t-q6CDz-1]) *)
fun every xs () k = streamConcatMap k xs
val run = ()
(* streams (BUG: software can't tell where this code came from [NWFRe5t-q6CDz-2]) *)
fun cartesian xs ys =
  every xs run (fn x => 
    every ys run (fn y =>
      streamOfList [(x, y)]))
...
(* streams (BUG: software can't tell where this code came from [NWFRe5t-q6CDz-3]) *)
...
(* streams (BUG: software can't tell where this code came from [NWFRe5t-q6CDz-4]) *)
...
fun setup_error_format interactivity =
  if prompts interactivity then
    ...
    before override_if_testing () (*OMIT*)
(* function application with overflow checking S81 *)
local
...
(* abstract syntax for \uprolog (BUG: software can't tell where this code came from [NWFRe5t-3z6K9k-1]) *)
(* definitions of [[term]], [[goal]], and [[clause]] for \uprolog (BUG: software can't tell where this code came from [NWFRe5t-39JBNL-1]) *)
datatype term = VAR     of name
              | LITERAL of int
              | APPLY   of name * term list
(* definitions of [[term]], [[goal]], and [[clause]] for \uprolog (BUG: software can't tell where this code came from [NWFRe5t-39JBNL-2]) *)
type goal = name * term list
(* definitions of [[term]], [[goal]], and [[clause]] for \uprolog (BUG: software can't tell where this code came from [NWFRe5t-39JBNL-3]) *)
datatype clause = :- of goal * goal list
infix 3 :-
(* definitions of [[def]] and [[unit_test]] for \uprolog (BUG: software can't tell where this code came from [NWFRe5t-1DUbAo-1]) *)
datatype cq
  = ADD_CLAUSE of clause
  | QUERY      of goal list
type def = cq
(* definitions of [[def]] and [[unit_test]] for \uprolog (BUG: software can't tell where this code came from [NWFRe5t-1DUbAo-2]) *)
datatype unit_test 
...
(* definitions of [[termString]], [[goalString]], and [[clauseString]] (BUG: software can't tell where this code came from [NW1DmjjU-2vN8ay-1]) *)
...
and appString f h t =
      String.concat (f :: "(" :: termString h ::
                     foldr (fn (t, tail) => ", " :: termString t :: tail) [")"]
                                                                              t)
(* definitions of [[termString]], [[goalString]], and [[clauseString]] (BUG: software can't tell where this code came from [NW1DmjjU-2vN8ay-2]) *)
...
(* definitions of [[termString]], [[goalString]], and [[clauseString]] (BUG: software can't tell where this code came from [NW1DmjjU-2vN8ay-3]) *)
...
(* support for tracing \uprolog\ computation (BUG: software can't tell where this code came from [NW1DmjjU-1QwTgg-1]) *)
...
(* substitution and unification ((upr)) (BUG: software can't tell where this code came from [NWFRe5t-bXWDy-1]) *)
datatype con = ~  of term * term
             | /\ of con  * con
             | TRIVIAL
infix 4 ~
infix 3 /\
(* free variables of terms, goals, clauses (BUG: software can't tell where this code came from [NWFRe5t-1jkaAg-1]) *)
fun termFreevars t =
...
(* substitutions for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-38kcqf-1]) *)
type subst = term env
val idsubst = emptyEnv
(* type declarations for consistency checking *)
type subst = subst
val _ = op idsubst : subst
(* substitutions for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-38kcqf-2]) *)
fun varsubst theta = 
  (fn x => find (x, theta) handle NotFound _ => VAR x)
(* type declarations for consistency checking *)
val _ = op varsubst : subst -> (name -> term)
(* substitutions for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-38kcqf-3]) *)
fun termsubst theta =
...
(* substitutions for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-38kcqf-4]) *)
...
(* substitutions for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-38kcqf-5]) *)
fun consubst theta =
...
(* substitutions for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-38kcqf-6]) *)
infix 7 |-->
...
(* substitutions for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-38kcqf-7]) *)
...
type con = con
val _ = op consubst    : subst -> (con -> con)
(* substitution and unification ((upr)) (BUG: software can't tell where this code came from [NWFRe5t-bXWDy-2]) *)
exception Unsatisfiable
(* constraint solving ((prototype)) (BUG: software can't tell where this code came from [NWFRe5t-3oQZQN-1]) *)
...
(* renaming \uprolog\ variables (BUG: software can't tell where this code came from [NWFRe5t-2bq7N9-1]) *)
local
  val n = ref 1
in
...
(* renaming \uprolog\ variables (BUG: software can't tell where this code came from [NWFRe5t-2bq7N9-2]) *)
fun freshen c =
...
(* lexical analysis and parsing for \uprolog, providing [[cqstream]] (BUG: software can't tell where this code came from [NW1DmjjU-4ZgiUT-1]) *)
(* lexical analysis for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-2Zex5K-1]) *)
datatype token 
  = UPPER     of string
  | LOWER     of string
  | SYMBOLIC  of string
...
(* type declarations for consistency checking *)
type token = token
(* lexical analysis for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-2Zex5K-2]) *)
...
(* lexical analysis for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-2Zex5K-3]) *)
...
fun lower "is" = RESERVED "is"
  ...
  | lower s    = LOWER s
(* lexical analysis for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-2Zex5K-4]) *)
fun anonymousVar () =
  case freshVar ""
    of VAR v => UPPER v
     | _ => let exception ThisCan'tHappen in raise ThisCan'tHappen end
(* lexical analysis for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-2Zex5K-5]) *)
local

(* character-classification functions for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-2kjf2p-1]) *)
  val symbols = explode "!%^&*-+:=|~<>/?`$\\"
...
(* lexical utility functions for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-2DYZQV-1]) *)
...
(* lexical utility functions for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-2DYZQV-2]) *)
...
(* lexical utility functions for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-2DYZQV-3]) *)
  fun nextline (file, line) = (file, line+1)
  (* type declarations for consistency checking *)
  val _ = op nextline : srcloc -> srcloc
in

(* lexical analyzers for for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-jzKpw-1]) *)
...
(* lexical analyzers for for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-jzKpw-2]) *)
  fun manySat p =
    many (sat p char)
...
(* lexical analyzers for for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-jzKpw-3]) *)
  val ordinaryToken =
...
(* lexical analyzers for for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-jzKpw-4]) *)
  local
...
(* parsers and streams for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-1e6gNN-1]) *)
...
(* parsers and streams for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-1e6gNN-2]) *)
val notSymbol =
  symbol <!> "arithmetic expressions must be parenthesized" <|>
  pure ()
(* type declarations for consistency checking *)
val _ = op notSymbol : unit parser
(* parsers and streams for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-1e6gNN-3]) *)
fun nilt tokens = pure (APPLY ("nil", [])) tokens
fun cons (x, xs) = APPLY ("cons", [x, xs])
(* type declarations for consistency checking *)
val _ = op nilt : term parser
val _ = op cons : term * term -> term
(* parsers and streams for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-1e6gNN-4]) *)
val variable        = upper
...
(* parsers and streams for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-1e6gNN-5]) *)
...
(* parsers and streams for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-1e6gNN-6]) *)
...
(* parsers and streams for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-1e6gNN-7]) *)
datatype concrete
  = BRACKET of string 
  | CLAUSE  of goal * goal list option
  | GOALS   of goal list
  | CTEST   of unit_test
(* parsers and streams for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-1e6gNN-8]) *)
...
(* parsers and streams for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-1e6gNN-9]) *)
val unit_test =
...
(* parsers and streams for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-1e6gNN-A]) *)
val notClosing =
...
(* parsers and streams for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-1e6gNN-B]) *)
...
(* parsers and streams for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-1e6gNN-C]) *)
datatype xdef_or_mode
  = XDEF of xdef
  | NEW_MODE of mode
(* type declarations for consistency checking *)
type xdef_or_mode = xdef_or_mode
(* parsers and streams for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-1e6gNN-D]) *)
...
(* parsers and streams for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-1e6gNN-E]) *)
val skippable = 
...
(* parsers and streams for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-1e6gNN-F]) *)
...
(* utility functions for [[cqstream]] (BUG: software can't tell where this code came from [NW1DmjjU-279YOV-1]) *)
...
(* utility functions for [[cqstream]] (BUG: software can't tell where this code came from [NW1DmjjU-279YOV-2]) *)
...
(* utility functions for [[cqstream]] (BUG: software can't tell where this code came from [NW1DmjjU-279YOV-3]) *)
        ...
        ; if startsWithEOF tokens then
            ...
            case xdef_or_mode mode tokens
              of SOME (OK (XDEF d),        tokens) => SOME (d, (ps1, mode,
               ...
               | NONE =>
(* fail epically with a diagnostic about [[tokens]] (BUG: software can't tell where this code came from [NW1DmjjU-2j4pdB-1]) *)
...
(* evaluation, testing, and the read-eval-print loop for \uprolog (BUG: software can't tell where this code came from [NWFRe5t-45lTnZ-1]) *)
(* \uprolog's database of clauses (BUG: software can't tell where this code came from [NWFRe5t-a3YFs-1]) *)
...
(* functions [[eval]], [[is]], and [[compare]], used in primitive predicates (BUG: software can't tell where this code came from [NWFRe5t-2UfWMN-1]) *)
...
(* functions [[eval]], [[is]], and [[compare]], used in primitive predicates (BUG: software can't tell where this code came from [NWFRe5t-2UfWMN-2]) *)
fun is [x, e] succ fail = (succ (solve (x ~ LITERAL (eval e))) fail
                           handle Unsatisfiable => fail())
  | is _      _    fail = fail ()
(* functions [[eval]], [[is]], and [[compare]], used in primitive predicates (BUG: software can't tell where this code came from [NWFRe5t-2UfWMN-3]) *)
...
(* tracing functions (BUG: software can't tell where this code came from [NWFRe5t-2SxcPU-1]) *)
...
(* search ((prototype)) (BUG: software can't tell where this code came from [NWFRe5t-1yDpEe-1]) *)
fun 'a query database =
  let val primitives = foldl (fn ((n, p), rho) => bind (n, p, rho))
                       emptyEnv (
(* \uprolog's primitive predicates [[::]] (BUG: software can't tell where this code came from [NWFRe5t-1ot2Wo-1]) *)
                                 ("true", fn args => fn succ => fn fail =>
                                            if null args then succ idsubst fail
                                                                else fail ()) ::

(* \uprolog's primitive predicates [[::]] (BUG: software can't tell where this code came from [NWFRe5t-1ot2Wo-2]) *)
                                 ("atom", fn args => fn succ => fn fail =>
                                             case args of [APPLY(f, [])] => succ
                                                                    idsubst fail
                                                        | _ => fail ()) ::

(* \uprolog's primitive predicates [[::]] (BUG: software can't tell where this code came from [NWFRe5t-1ot2Wo-3]) *)
...
(* \uprolog's primitive predicates [[::]] (BUG: software can't tell where this code came from [NWFRe5t-1ot2Wo-4]) *)
                                 ("is", is) ::

(* \uprolog's primitive predicates [[::]] (BUG: software can't tell where this code came from [NWFRe5t-1ot2Wo-5]) *)
                                 ("<",  compare "<"  op < ) ::
                                 (">",  compare ">"  op > ) ::
                                 ("=<", compare "=<" op <= ) ::
                                 (">=", compare ">=" op >= ) ::

(* \uprolog's primitive predicates [[::]] (BUG: software can't tell where this code came from [NWFRe5t-1ot2Wo-6]) *)
...
(* interaction (BUG: software can't tell where this code came from [NWFRe5t-3erztQ-1]) *)
...
(* definitions of [[basis]] and [[processDef]] for \uprolog (BUG: software can't tell where this code came from [NWFRe5t-2l2jjJ-1]) *)
type basis = database
fun processDef (cq, database, interactivity) =
  let fun process (ADD_CLAUSE c) = addClause (c, database)
        | process (QUERY gs) = (
(* query goals [[gs]] against [[database]] (BUG: software can't tell where this code came from [NWFRe5t-Y8oT1-1]) *)
                                query database gs
                                  (fn theta => fn resume =>
...
(* definition of [[testIsGood]] for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-29XpPZ-1]) *)
fun testIsGood (test, database) =
  let
(* definitions of [[checkSatisfiedPasses]] and [[checkUnsatisfiablePasses]] (BUG: software can't tell where this code came from [NW1DmjjU-38y5hK-1]) *)
...
(* definitions of [[checkSatisfiedPasses]] and [[checkUnsatisfiablePasses]] (BUG: software can't tell where this code came from [NW1DmjjU-38y5hK-2]) *)
...
(* definitions of [[checkSatisfiedPasses]] and [[checkUnsatisfiablePasses]] (BUG: software can't tell where this code came from [NW1DmjjU-38y5hK-3]) *)
...
(* definitions of [[checkSatisfiedPasses]] and [[checkUnsatisfiablePasses]] (BUG: software can't tell where this code came from [NW1DmjjU-38y5hK-4]) *)
...
(* definition of [[useFile]], to read from a file (BUG: software can't tell where this code came from [NWFRe5t-5qpVN-1]) *)
            val try = useFile
            fun useFile filename = 
...
(* function [[runAs]] for \uprolog (BUG: software can't tell where this code came from [NW1DmjjU-3s7P9t-1]) *)
...
(* code that looks at \uprolog's command-line arguments and calls [[runAs]] (BUG: software can't tell where this code came from [NW1DmjjU-3g18Gs-1]) *)
