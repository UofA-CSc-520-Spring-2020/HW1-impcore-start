...
Exp reduce_to_exp(int code, struct Component *components) {
    ...
    /* cases for \impcore's [[reduce_to_exp]] added in exercises S37d */
    /* add your syntactic extensions here */
    /* cases for \impcore's [[reduce_to_exp]] added in exercises S52b */
...
/* arrays of shift functions added to \impcore\ in exercises S46c */
/* for each new row added to exptable, add an array of shift functions here */
/* arrays of shift functions added to \impcore\ in exercises S53a */
...
struct ParserRow exptable[] = {
  ...
  /* rows added to \impcore's [[exptable]] in exercises S46d */
  /* add a row here for each new syntactic form of Exp */
  /* rows added to \impcore's [[exptable]] in exercises S53b */
...
struct Usage usage_table[] = {
    ...
    { BEGIN,  "(begin exp ... exp)" },
    /* \impcore\ [[usage_table]] entries added in exercises S53c */
    { SUGAR(CAND), "(&& exp exp)" },
    { SUGAR(COR),  "(|| exp exp)" },
